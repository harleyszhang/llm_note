...
为了做性能分析，`Roofline` 模型论文引入了“操作强度”（operational intensity, 也叫算术强度）这个概念，表示每字节 `DRAM/HBM` 数据传输（traffic）所对应的操作次数（每字节 `flops`），这里的字节数是指那些经过缓存层过滤后进入主内存 `DRAM/HBM` 的字节，即我们关注的是缓存与内存之间的数据传输，而不是处理器与缓存之间的数据传输（traffic）。因此，操作强度反映了在特定计算机上某个计算核所需的 DRAM 带宽。公式定义如下：
...
`Roofline` 模型曲线图的 X 轴表示 `Flops/byte`（GFlops per byte）即**操作强度**，Y 轴表示 GFlops/s（GFlops per second）。
