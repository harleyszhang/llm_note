...
- [4. Transformer 模型的张量并行](#4-transformer-模型的张量并行)
...
基于 transformer 网络 pytorch 代码的基础上，通常只需添加几个**同步操作代码**（synchronization primitives），就可实现一个简单的模型并行方案。
