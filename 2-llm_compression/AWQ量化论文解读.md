...
- [2. AWQ: 激活感知的权重量化](#2-awq-激活感知的权重量化)
  - [2.1 保留 `1%` 的关键权重来提升 LLM 量化性能](#21-保留-1-的关键权重来提升-llm-量化性能)
  - [2.2 基于激活感知缩放保护关键权重](#22-基于激活感知缩放保护关键权重)
- [3. 实验](#3-实验)
- [4. 结论](#4-结论)
- [参考资料](#参考资料)
...
现在考虑对于一个权重元素 $w \in \mathbf{w}$，如果我们引入缩放因子 $s$，并在量化过程中将 $w$ 与 $s$ 相乘同时将激活 $x$ 以同样的缩放因子 $s$ 逆向缩放！即 $Q(w \cdot s)(x / s)$，量化函数形式为：
...
$\Delta'$ 是在应用 $s$ 之后的新量化缩放系数。虽然公式 1 和公式 2 在数学上是“等效”的，但是带来的精度损失是不一样的。作者通过实验发现：
...
新误差与原始误差的比率为 $\frac{\Delta{\prime}}{\Delta} \cdot \frac{1}{s}$。因为 $\Delta{\prime} \approx \Delta$ 且 $s > 1$，所以显著权重 $w$ 的相对误差较小。

为了验证该想法，作者在 OPT-6.7B 模型的 `1%` 显著通道上乘以 $s > 1$，并在表 2 中展示每个组中 $\Delta$ 的变化情况。结果显示，放大显著通道非常有效：困惑度从 $s = 1$ 时的 23.54（仅为 RTN）降低到 $s = 2$ 时的 11.92。随着 $s$ 增加，$\Delta$ 的变化比例逐渐增大，但在 $s < 2$ 时（小于 5%）依然较小；显著通道的相对误差随 $s$ 增加持续减小。然而，最佳困惑度在 s = 2 时出现。这是因为如果 $s$ 过大，$\Delta$ 增加会放大非显著通道的相对误差（非显著通道的误差会被 $\frac{\Delta{\prime}}{\Delta}$ 放大，在 $s = 4$ 时 21.2% 的通道比率大于 1），从而可能影响模型整体精度。**因此，在保护显著通道时，我们还需考虑非显著通道的误差**。
